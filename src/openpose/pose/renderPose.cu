...
namespace op
{
    ...
    __global__ void renderPoseBody21(float* targetPtr, const int targetWidth, const int targetHeight,
                                      const float* const posePtr, const int numberPeople, const float threshold,
                                      const bool googlyEyes, const bool blendOriginalFrame, const float alphaColorToAdd)
    {
        const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
        ...
        // Other parameters
        const auto numberPartPairs = sizeof(BODY_21_PAIRS_GPU) / (2*sizeof(BODY_21_PAIRS_GPU[0]));
        const auto numberScales = sizeof(BODY_21_SCALES) / sizeof(BODY_21_SCALES[0]);
        const auto numberColors = sizeof(BODY_21_COLORS) / (3*sizeof(BODY_21_COLORS[0]));
        const auto radius = fastMin(targetWidth, targetHeight) / 100.f;
        const auto lineWidth = fastMin(targetWidth, targetHeight) / 120.f;

        // Render key points
        renderKeypoints(targetPtr, sharedMaxs, sharedMins, sharedScaleF, globalIdx, x, y, targetWidth, targetHeight,
                        posePtr, BODY_21_PAIRS_GPU, numberPeople, 21, numberPartPairs, BODY_21_COLORS, numberColors,
                        radius, lineWidth, BODY_21_SCALES, numberScales, threshold, alphaColorToAdd,
                        blendOriginalFrame, (googlyEyes ? 15 : -1), (googlyEyes ? 16 : -1));
    }
                                ...
                                const bool googlyEyes, const bool blendOriginalFrame, const float alphaBlending)
    {
        try
        {
            if (numberPeople > 0 || !blendOriginalFrame)
            {
                ...
                else if (poseModel == PoseModel::BODY_21 || poseModel == PoseModel::BODY_21_X2)
